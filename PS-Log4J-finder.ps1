...
get-wmiobject win32_volume|? {$_.DriveType-eq3}|% {(Get-Psdrive  $_.DriveLetter[0]).Name}|Foreach {$command='findstr /i /s /m "SocketServer.class JndiLookup.class" {0}:\*.jar >> "{1}\log4j1.log" | findstr /i /s /m "SocketServer.class JndiLookup.class" {0}:\*.war >> "{1}\log4j2.log" | findstr /i /s /m "SocketServer.class JndiLookup.class" {0}:\*log4j*.dll  >> "{1}\log4j3.log" | findstr /i /s /m "SocketServer.class JndiLookup.class" {0}:\*.ear >> "{1}\log4j4.log"'-f $_, $Env:TEMP;cmd /c $command};
